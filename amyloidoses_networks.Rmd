...
Remember to set your working directory to where your data are located by changing this line of code.
```{r setup}
knitr::opts_knit$set(root.dir = '~/Desktop/R_scripts')
```
...
if(!file.exists(network_file)) {
  download.file("https://jensenlab.org/assets/amyco/amyco_disease_protein_interactions.tsv", network_file)
...
if(!file.exists(attribute_file)) {
  download.file("https://jensenlab.org/assets/amyco/protein_or_disease_amyco.txt", attribute_file)
...
for (i in diseases_node_suids){
  ...
  if (length(first_neighbors$nodes) !=1){
    ...
    if(flag==0){
      ...
      Sys.sleep(0.5)
      #set node size for the other nodes to 30, mapping style= discrete
      #setNodeSizeMapping("query term","",30,mapping.type="d",style.name = "AmyCo style")
      #Sys.sleep(1)
    }
    Sys.sleep(0.5)
  ...
  ##Select main network
  setCurrentNetwork("All_interactions_no_app_prnp")
}
```
##Save session
```{r}
